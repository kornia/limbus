...
class _ComponentState():
    ...
    The user interactions are the ones allowing simultaneous states, concretelly:
        ...
        - ComponentState.RECEIVING_EVENTS (this is the exception, it is not due to a user interaction, it must accept
                                           other states since sometimes it is setted after finishing the execution of
                                           the forward method and then we could lose the execution state)
...
class Component(base_class):
    ...
    def __init__(self, name: str):
        ...
        for event in self._input_events:
            if event.type is param.BeforeComponentCallEventType:
                self.__events_to_wait_before_call.__setattr__(event.name, event)
            elif event.type is param.BeforeComponentIterEventType:
                self.__events_to_wait_before_running.__setattr__(event.name, event)
            elif event.type is param.AfterComponentIterEventType:
